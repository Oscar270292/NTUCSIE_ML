...
for station_id in station:
    week1_mon = data_process.read_data("20231003", station_id)
    week2_mon = data_process.read_data("20231010", station_id)
    week3_mon = data_process.read_data("20231017", station_id)
    week5_mon = data_process.read_data("20231031", station_id)
    week6_mon = data_process.read_data("20231107", station_id)
    week7_mon = data_process.read_data("20231114", station_id)
    week8_mon = data_process.read_data("20231121", station_id)
    result = pd.concat([week1_mon, week2_mon, week5_mon, week6_mon, week7_mon,week8_mon, week3_mon])

    big, small = result["sbi"].max(), result["sbi"].min()
    data_nor = data_process.normalize(result)
    grouped = [data_nor.iloc[ i:i + 72, :] for i in range(0, 72*7, 72)]
    nor_list = []
    ...
    data = {'id': data_process.id_list(24, station_id), 'sbi': data_frame['sbi']}
    df = pd.DataFrame(data)
    data_frame_final = pd.concat([data_frame_final, df])

data_frame_final.to_csv('20231024_1.csv', index=False)
