...
model.fit(x_train, y_train, epochs=3, batch_size=5, callbacks=[callback])
...
plt.plot(y_train.flatten(), color='red', label='Real')  # 紅線表示真實股價
plt.plot(lstm_pre.flatten(), color='blue', label='Predicted')  # 藍線表示預測股價
