...
def main(train_url, test_url):
    ...
    trees, e_ins, e_outs, e_outs_forest = random_forest(X_train, y_train, X_test, y_test, n_trees=5)
    ...
    plt.show()
    plt.savefig('./hw6_10_histogram.png')  # 儲存圖片
    ...
    plt.show()
    plt.savefig('./hw6_11_scatter.png')  # 儲存圖片

    # 繪製 Eout(gt) 和 Eout(Gt) 作為 t 的函數
    plt.figure()
    plt.plot(e_outs, label='Eout(gt)')
    plt.plot(e_outs_forest, label='Eout(Gt)')
    plt.xlabel('Number of Trees')
    plt.ylabel('Eout')
    plt.title('Eout(gt) vs Eout(Gt)')
    plt.legend()
    plt.show()
    plt.savefig('./hw6_12_eout_vs_t.png')  # 儲存圖片

    print('Histogram, scatter plot, and Eout vs. t plots have been saved.')
